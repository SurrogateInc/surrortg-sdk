...
2. [create a game on surrogate.tv](#create-a-game-instance-on-surrogate-tv)
3. [install controller sdk](#installing-surrogate-controller)
4. [install streamer](#installing-surrogate-streamer)
5. [install watcher stream](#installing-surrogate-watcher-stream)
6. [configure your streamer and controller](#configuration-file)
7. [test simple game](#running-template-game)
8. [create systemd unit from controller](#running-the-surrortg-python-sdk-controller-automatically-on-boot)

You might need to jump between steps 5-7 in case you missed something so remember to look around if you run into trouble. Also take a look at the [troubleshooting page](troubleshooting). There can also find out how to read logs and restart the code modules.
...
The watcherview components creates a systemd unit `srtg-watcherstream`, which you can control similarly to other systemd units.
...
You can see how to start your controller code automatically on boot [here](#running-the-surrortg-python-sdk-controller-automatically-on-boot).
...
<strong>Note: if you want that your code runs constantly and starts automatically after on-boot, you should setup a systemd unit [this way](#running-the-surrortg-python-sdk-controller-automatically-on-boot). </strong>
