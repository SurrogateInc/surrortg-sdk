...
from surrortg import Game, ConfigType
from surrortg.inputs import Joystick, LinearActuator, KeyCode
from games.surrobot.hw import Hw
from games.surrobot.template_games import Game1, RacingGame, Game3
...
class SurrobotGame(Game):
    async def on_init(self):
        ...
        # extra configuration spesific to game type. Every game-type has it's own game loop
        # implementation. for e.g. racing-game requires 1 light sensor, 4 wheel or 2 wheel drive
        ...
        self.io.register_config(
            "game-template-2[conditional]-max-laps", ConfigType.INTEGER, 3, False, minimum=1, maximum=10
        )
        ...
        self.io.register_config(
            "movement-speed", ConfigType.INTEGER, 8, False, minimum=1, maximum=10
        )
        ...
        motor_joystick = MotorJoystick(self.hw.motor_controller, custom_binds=[
            {
                "up": KeyCode.KEY_W,
                "down": KeyCode.KEY_S,
                "left": KeyCode.KEY_A,
        ...
        camera = ServoJoystick(self.servos[0], self.servos[1], [
            {
        ...
        claw = ServoActuator(self.servos[2], [
            {
                "min": "KeyN",
                "max": "KeyM",
            }
